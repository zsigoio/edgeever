...
`create_diagram_memo` MCP 工具已接收这层语义创建 IR：节点标识、名称、语义类型、容器关系与连接关系。EdgeEver 会生成连线标识、节点尺寸、坐标、确定性布局和架构边界几何信息，再编译为持久化 `DiagramDocument`。可选的布局方向只是提示，不是由 Agent 编排的几何数据。
